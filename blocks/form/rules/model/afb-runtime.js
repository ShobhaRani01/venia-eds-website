...
import {
  propertyChange, ExecuteRule, Initialize, RemoveItem, Change, FormLoad, FieldChanged, ValidationComplete, Valid, Invalid, SubmitSuccess, CustomEvent, SubmitError, SubmitFailure, Submit, Save, Reset, RemoveInstance, AddInstance, AddItem, Click,
} from './afb-events.js';
import Formula from '../formula/index.js';
import {
  format, parseDefaultDate, datetimeToNumber, parseDateSkeleton, numberToDatetime, formatDate, parseDate,
...
const customConstraintTypeMessages = {};
...
const isRepeatable$1 = (obj) => ((obj.repeatable
...
class DataGroup extends DataValue {
  ...
  constructor(_name, _value, _type = typeof _value, parent) {
    ...
    if (_value instanceof Array) {
      this.$_items = _value.map((value, index) => this.createEntry(index, value, this));
    } else {
      this.$_items = Object.fromEntries(Object.entries(_value).map(([key, value]) => [key, this.createEntry(key, value, this)]));
    }
  }
  ...
  get $value() {
    ...
    return Object.fromEntries(Object.values(this.$_items).filter((x) => typeof x !== 'undefined' && !x.disabled).map((x) => [x.$name, x.$value]));
  }
...
const isGlobal = (prev, stream, pos) => prev === null && stream[pos] === globalStartToken;
const isRepeatable = (prev, stream, pos) => prev === null && stream[pos] === repeatableStartToken;
...
const tokenize = (stream) => new Tokenizer(stream).tokenize();
const resolveData = (data, input, create) => {
  ...
  let result = data;
  let i = 0;
  const createIntermediateNode = (token, nextToken, create) => (nextToken === null ? create
  ...
  return items?.reduce((acc, item) => {
    ...
    if (item.isContainer) {
      ...
      if (item.value instanceof Array) {
        ret[item.id] = item.value.map((x) => ({ ...x, dataRef }));
      } else if (item.value != null) {
        ret[item.id] = { ...item.value, dataRef };
      }
    }
...
const isFormOrSiteContainer = (model) => (':items' in model || 'cqItems' in model) && (':itemsOrder' in model || 'cqItemsOrder' in model);
...
const replaceTemplatePlaceholders = (str, values = []) => str?.replace(/\${(\d+)}/g, (match, index) => {
...
const matchMediaType = (mediaType, accepts) => !mediaType || accepts.some((accept) => {
...
const Constraints = {
  ...
  minimum: (constraint, value) => ({ valid: value >= constraint, value }),
  maximum: (constraint, value) => ({ valid: value <= constraint, value }),
  exclusiveMinimum: (constraint, value) => ({ valid: value > constraint, value }),
  exclusiveMaximum: (constraint, value) => ({ valid: value < constraint, value }),
  minItems: (constraint, value) => ({ valid: (value instanceof Array) && value.length >= constraint, value }),
  maxItems: (constraint, value) => ({ valid: (value instanceof Array) && value.length <= constraint, value }),
  uniqueItems: (constraint, value) => ({ valid: !constraint || ((value instanceof Array) && value.length === new Set(value).size), value }),
  minLength: (constraint, value) => ({ ...Constraints.minimum(constraint, typeof value === 'string' ? value.length : 0), value }),
  maxLength: (constraint, value) => ({ ...Constraints.maximum(constraint, typeof value === 'string' ? value.length : 0), value }),
...
class BaseNode {
  ...
  setupRuleNode() {
    ...
    this._ruleNode = new Proxy(this.ruleNodeReference(), {
      get: (ruleNodeReference, prop) => self.getFromRule(ruleNodeReference, prop),
    });
  }
  ...
  getFromRule(ruleNodeReference, prop) {
    ...
    if (typeof (prop) === 'string') {
      if (prop.startsWith('$')) {
        ...
        if (typeof ruleNodeReference[prop] === 'function') {
          return ruleNodeReference[prop];
        }
      }
    }
  ...
  _addDependent(dependent) {
    if (this._dependents.find(({ node }) => node === dependent) === undefined) {
      const subscription = this.subscribe((change) => {
        ...
        const isPropChanged = changes.findIndex((x) => propsToLook.indexOf(x.propertyName) > -1) > -1;
        if (isPropChanged) {
  ...
  bindToDataModel(contextualDataModel) {
    ...
    const { dataRef } = this._jsonModel;
    let _data; let _parent = contextualDataModel; let
      _key = '';
    if (dataRef === null) {
      _data = NullDataValue;
    ...
    } else if (contextualDataModel !== NullDataValue && staticFields.indexOf(this.fieldType) === -1) {
...
class Container extends Scriptable {
  ...
  getItemsState(isRepeatableChild = false, forRestore = false) {
    if (this._jsonModel.type === 'array' || isRepeatable$1(this._jsonModel) || isRepeatableChild) {
      ...
      return this._children.map((x) => ({ ...x.getState(true, forRestore) }));
    }
  ...
  _addChild(itemJson, index, cloneIds = false, mode = 'create') {
    ...
    const itemTemplate = {
      index,
      ...deepClone(itemJson, cloneIds ? () => form.getUniqueId() : undefined),
    };
  ...
  validate() {
    return this.items.flatMap((x) => x.validate()).filter((x) => x.fieldName !== '');
  }
  ...
  syncDataAndFormModel(contextualDataModel) {
    ...
    if (contextualDataModel?.$type === 'array' && this._itemTemplate != null) {
      ...
      const { minItems } = this._jsonModel;
...
const submit = async (context, success, error, submitAs = 'multipart/form-data', input_data = null, action = '', metadata = null) => {
  ...
  const submitDataAndMetaData = { data, ...metadata };
...
class FunctionRuntimeImpl {
          ...
          _func: (args, data, interpreter) => {
            const globals = {
              ...
              functions: {
                ...
                exportData: () => FunctionRuntimeImpl.getInstance().getFunctions().exportData._func.call(undefined, args, data, interpreter),
                ...
                markFieldAsInvalid: (fieldIdentifier, validationMessage, option) => {
                  if (!option || option.useId) {
                    ...
                    interpreter.globals.form.visit((f) => {
  ...
  getFunctions() {
    ...
    const defaultFunctions = {
      ...
      exportData: {
        _func: (args, data, interpreter) => interpreter.globals.form.exportData(),
        _signature: [],
      },
      importData: {
        _func: (args, data, interpreter) => {
      ...
      submitForm: {
        _func: async (args, data, interpreter) => {
          ...
          const { form } = interpreter.globals;
...
class Field extends Scriptable {
  ...
  get emptyValue() {
    ...
    if (this._jsonModel.emptyValue === '' && this.type === 'string') {
      return '';
    }
  ...
  checkStep() {
    ...
    if (typeof step === 'number') {
      ...
      const valid = Math.abs(fVal - fIVal) % fStep < 0.001;
      let next; let
        prev;
      if (!valid) {
...
class DateField extends Field {
  ...
  set value(value) {
    ...
    } else if (this._jsonModel.editFormat !== 'short' && this._jsonModel.editFormat !== 'date|short') {
...
class FormFieldFactoryImpl {
  createField(child, _options) {
    ...
    } else if (isFile(child) || child.fieldType === 'file-input') {
...
export {
  createFormInstance, fetchForm, registerFunctions, restoreFormInstance, validateFormData, validateFormInstance,
};
