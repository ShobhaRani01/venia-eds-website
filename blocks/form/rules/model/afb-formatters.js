...
const DATE_TIME_REGEX = /(?:[Eec]{1,6}|G{1,5}|[Qq]{1,5}|(?:[yYur]+|U{1,5})|[ML]{1,5}|d{1,2}|D{1,3}|F{1}|[abB]{1,5}|[hkHK]{1,2}|w{1,2}|W{1}|m{1,2}|s{1,2}|[zZOvV]{1,5}|[zZOvVxX]{1,3}|S{1,3}|'(?:[^']|'')*')|[^a-zA-Z']+/g;
...
function getTimezoneOffsetFrom(otherTimezone) {
  ...
  let str = date.toLocaleString('en-US', {
    timeZone: otherTimezone, day: 'numeric', hour: 'numeric', minute: 'numeric', hourCycle: 'h23',
  });
  ...
  const otherOffset = (other.day * 24 * 60) + (other.hour * 60) + (other.minute);
...
function parseDate(dateString, language, skeleton, timeZone, bUseUTC = false) {
  ...
  parsed.forEach(([option, value, len]) => {
    if (option === 'literal') {
      ...
      regexParts.push(value === 'numeric' ? fourDigit : twoDigit);
...
const NUMBER_REGEX = /(?:[#]+|[@]+(#+)?|[0]+|[,]|[.]|[-]|[+]|[%]|[¤]{1,4}(?:\/([a-zA-Z]{3}))?|[;]|[K]{1,2}|E{1,2}[+]?|'(?:[^']|'')*')|[^a-zA-Z']+/g;
...
export {
  datetimeToNumber, format, formatDate, formatNumber, numberToDatetime, parse, parseDate, getSkeleton as parseDateSkeleton, parseDefaultDate, parseNumber,
};
